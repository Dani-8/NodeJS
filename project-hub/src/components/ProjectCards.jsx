    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {projects.map((project) => (
                <div key={project.id}
                    className={`group relative bg-[#121215] border border-white/5 rounded-[2.5rem] p-8 ${project.boderOnHover} transition-all duration-500 hover:-translate-y-2 overflow-hidden`}
                >
                    {/* Gradient Glow */}
                    ...
                    <div className="relative z-10 flex flex-col h-full">
                        ...
                        <h3 className={`text-2xl font-bold text-white mb-3 ${project.titleOnHover} transition-colors duration-300`}>
                            {project.title}
                        </h3>
